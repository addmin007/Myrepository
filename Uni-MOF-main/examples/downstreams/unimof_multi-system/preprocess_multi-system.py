...
def single_parser(content):
    dir_path = '/root/Uni-MOF-main/Uni-MOF-main/data/S1-CIF' # replace to your MOF database path
    cif_name, gas, gas_attr, temperature, pressure, targets, task_name, structure_features = content
...
def get_data(path):
    ...
    cif_names = 'MOF'
    gas = 'gas-name'
    gas_attr_cols = ['gas-critical-temperature', 'gas-critical-pressure', 'gas-acentric-factor', 'gas-molar-mass']
    temperature = 'temperature'
    pressure = 'pressure'
    columns = 'target-value'

    gas_name_to_id = {'CH4': 1, 'CO2': 2, 'Ar': 3, 'Kr': 4, 'Xe': 5, 'O2': 6, 'N2': 7}
    data['gas_id'] = data[gas].map(gas_name_to_id)

    structure_feature_cols = ['LCD (Å)', 'PLD (Å)', 'LFPD (Å)', 'Density (g/cm3)', 'ASA (m2/cm3)', 'ASA (m2/g)', 'VF']
    ...
    return [(item[0], item[1], item[2], item[3], item[4], item[5], item[6], item[7])
            for item in zip(data[cif_names], data['gas_id'], data[gas_attr_cols].values,
                           data[temperature], data[pressure], data[columns],
                           data['task_name'], data[structure_feature_cols].values)]
...
if __name__ == '__main__':
    ...
    inpath = '/root/Uni-MOF-main/Uni-MOF-main/data/S1.csv' # replace to your data path
    outpath = '/root/Uni-MOF-main/Uni-MOF-main/data' # replace to your out path
