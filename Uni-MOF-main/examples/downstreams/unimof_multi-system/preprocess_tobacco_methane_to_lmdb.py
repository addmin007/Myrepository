...
def get_structure_features(obj):
    ...
    lfpd = float(obj.get("lfpd", 0.0) or 0.0)  # lfpd absent in Tobacco dataset; fill 0 rather than duplicating pld
...
def interpolate_isotherm(pressures, adsorptions, n_interp=2):
    """
    Insert n_interp interpolated pressure points between each pair of
    original measurement points using log-linear interpolation.
    Returns list of (pressure, adsorption) tuples including originals.

    Log-linear interpolation: pressure on log scale, adsorption on linear scale.
    This respects the physical shape of adsorption isotherms better than
    purely linear interpolation.
    """
    if len(pressures) < 2:
        return list(zip(pressures, adsorptions))

    # Sort by pressure
    pairs = sorted(zip(pressures, adsorptions), key=lambda x: x[0])
    result = []
    for i, (p1, q1) in enumerate(pairs):
        ...
        if i < len(pairs) - 1:
            p2, q2 = pairs[i + 1]
            if p1 > 0 and p2 > 0:
                log_p1, log_p2 = np.log10(p1), np.log10(p2)
                for k in range(1, n_interp + 1):
                    alpha = k / (n_interp + 1)
                    log_p_interp = log_p1 + alpha * (log_p2 - log_p1)
                    q_interp     = q1    + alpha * (q2    - q1)
                    if q_interp > 0:
                        result.append((10 ** log_p_interp, q_interp))
    return result


def build_rows_from_json_folder(data_dir: str, augment_interp: int = 2):
    """
    Parse JSON files and build row list.

    augment_interp: number of interpolated pressure points to insert between
                    each pair of original measurements (0 = disabled).
                    With the 3 original Tobacco pressures [6, 65, 100] bar and
                    augment_interp=2, each isotherm produces 3 + 2*2 = 7 rows.
    """
    rows = []
    ...
    for fname in sorted(os.listdir(data_dir)):
        ...
        try:
            ...
            for iso in isotherms:
                ...
                # Collect all (pressure, adsorption) pairs for this isotherm
                iso_data = {}   # gas_name -> [(pressure, adsorption), ...]
                ...
                for point in iso.get("isotherm_data", []) or []:
                    ...
                    for gas_name in iso_gases:
                        ...
                        if target_value is not None and target_value > 0:
                            iso_data[gas_name].append((pressure, target_value))

                for gas_name, pt_list in iso_data.items():
                    if not pt_list:
                        continue

                    # Augment with interpolated points
                    if augment_interp > 0 and len(pt_list) >= 2:
                        pressures_orig = [p for p, _ in pt_list]
                        adsorps_orig   = [q for _, q in pt_list]
                        pt_list = interpolate_isotherm(
                            pressures_orig, adsorps_orig, n_interp=augment_interp
                        )

                    gas_id   = GAS_NAME_TO_ID[gas_name]
                    gas_attr = GAS_FEATURES[gas_name]

                    for pressure, target_value in pt_list:
                        task_name = f"{cif_name}#{gas_name}#{temperature}#{pressure:.4f}"
                        rows.append((
                            cif_name,
                            gas_id,
                            gas_attr,
...
def main():
    parser = argparse.ArgumentParser(description="Convert Tobacco MOF dataset to Uni-MOF LMDB format")
    ...
    parser.add_argument("--augment_interp", type=int, default=2,
                        help="Number of interpolated pressure points between each original pair "
                             "(0=disabled, 2=recommended: 3 orig -> 7 points per isotherm)")
    args = parser.parse_args()

    rows = build_rows_from_json_folder(args.data_dir, augment_interp=args.augment_interp)
    print(f"total usable rows: {len(rows)}")
    if len(rows) == 0:
        raise RuntimeError("No valid rows parsed from json files. Please check data content.")

    # 只用原始压力点的目标值计算统计量（不含插值点），插值点不改变真实分布
    orig_rows = [r for r in rows if abs(r[4] - 6.0) < 0.1 or
                                    abs(r[4] - 65.0) < 0.1 or
                                    abs(r[4] - 100.0) < 0.1]
    targets = np.array([r[5] for r in orig_rows])
    value_log1p = np.log1p(targets)
    _mean, _std = value_log1p.mean(), value_log1p.std()
    print(f"mean and std of target values (log1p, original points only): {_mean}, {_std}")
    print(f"Please verify ATTR_REGESTRY in regloss.py has:")
