...
ATTR_REGESTRY = {
    ...
    'S1_CH4_1bar': [4.3757066134, 1.0909639617, 'log1p_standardization'],
    ...
    'CoRE': [1.318703908155812, 1.657051374039756,'log1p_standardization'],
...
class MOFV1MSELoss(UnicoreLoss):
    ...
    def reduce_metrics(logging_outputs, split='valid') -> None:
        ...
        if sample_size == 0:
            return
        num_tasks = logging_outputs[0].get("num_task", 1)
...
class MOFV2MSELoss(UnicoreLoss):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        """
        # 获取当前epoch
        if hasattr(self.task, 'trainer') and hasattr(self.task.trainer, 'epoch'):
            self.epoch = self.task.trainer.epoch
    ...
    def compute_loss(self, model, net_output, sample, normalizer, reduce=True):
        ...
        # ── 改进1：单调性物理先验损失 ─────────────────────────────────────────
        # 对同一 MOF 在不同压力点的预测，强制 q(P1) < q(P2) 当 P1 < P2
        # task_name 格式: "cifname#gas#temperature#pressure"
        # 只对同一 MOF（相同 cifname）的样本对施加约束，跨 MOF 无物理意义
        mono_loss = torch.tensor(0.0, device=predicts.device)
        if self.training and "pressure" in sample["net_input"] and "task_name" in sample:
            pressures = sample["net_input"]["pressure"].float()   # [B]
            pred_vals = normalizer.inverse_transform(predicts.squeeze(-1))  # [B], real space
            task_names = sample["task_name"]  # list of str, len=B

            # 提取 cifname（task_name 第一个 '#' 前的部分）
            mof_ids = [t.split('#')[0] if '#' in t else t for t in task_names]
            ...
            if B > 1:
                for i in range(B):
                    for j in range(i + 1, B):
                        # 只比较同一 MOF 的不同压力点
                        if mof_ids[i] != mof_ids[j]:
                            continue
                        dp = pressures[j] - pressures[i]
                        if dp > 0.1:   # p_j > p_i，要求 q_j >= q_i
                            violations.append(F.relu(pred_vals[i] - pred_vals[j]))
                        elif dp < -0.1:  # p_i > p_j，要求 q_i >= q_j
                            violations.append(F.relu(pred_vals[j] - pred_vals[i]))
                if violations:
                    mono_loss = torch.stack(violations).mean()
                    # 归一化到与 MSE 相近量级
                    mono_loss = mono_loss / (normalizer.std + 1e-6)

        # 总损失：MSE + 0.1 * 单调性惩罚
        mono_weight = getattr(self.args, 'mono_loss_weight', 0.1)
        loss = mse_loss + mono_weight * mono_loss
    ...
    def reduce_metrics(logging_outputs, split='valid') -> None:
        ...
        num_tasks = logging_outputs[0].get("num_task", 1)
        # we divide by log(2) to convert the loss from base e to base 2
        loss = loss_sum / sample_size / num_tasks
        metrics.log_scalar(
        ...
        if 'valid' in split or 'test' in split:
            ...
            # 获取epoch信息
            epoch = logging_outputs[0].get("epoch", 0)
            ...
            # 方式1：从环境变量获取
            if 'UNICORE_SAVE_DIR' in os.environ:
                save_dir = os.environ['UNICORE_SAVE_DIR']
            # 方式2：从当前工作目录的results文件夹
            elif os.path.isdir('./results'):
                save_dir = './results'
            # 方式3：检查常见的保存路径
            elif os.path.isdir('/root/Uni-MOF-main/Uni-MOF-main/results'):
                save_dir = '/root/Uni-MOF-main/Uni-MOF-main/results'
            ...
            with open(result_file, 'a') as f:
                f.write(f"Epoch {epoch}, {split}_loss: {loss:.6f}, {split}_r2: {r2:.6f}\n")
