...
class UniMOFV2Task(UnicoreTask):
    ...
    def build_model(self, args):
        ...
        if args.finetune_mol_model is not None:
                ...
                full_sd = state["model"]
                # Load backbone + gas/env/structure embed, but NOT classifier
                # (classifier was calibrated on high-pressure data and hurts 1-bar fine-tuning)
                transfer_sd = {
                    k: v for k, v in full_sd.items()
                    if not k.startswith("classifier.")
                }
                missing, unexpected = model.load_state_dict(transfer_sd, strict=False)
                print(f"transfer loaded (excl classifier): missing={len(missing)}, unexpected={len(unexpected)}")
                if missing:
